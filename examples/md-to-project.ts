...
async function run() {
  console.log("=== Sync Markdown Files to GitHub Project (v0.1.11) ===");

  const argDir = process.argv[2];
  const mdDir = argDir && !argDir.startsWith("-") ? path.resolve(argDir) : path.join(__dirname, "md");
  ...
  if (result.success) {
    console.log("Markdown directory synced successfully!");
    console.log(`Sync completed: ${result.processedFiles} markdown files processed (Created: ${result.created} stories, Skipped: ${result.skipped} duplicates).`);
  } else {
